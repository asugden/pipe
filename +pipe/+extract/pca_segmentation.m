function [ica_segments, segmentlabel, segcentroid] = pca_segmentation(ica_filters, smwidth, thresh, arealims, plotting, axon)
% Updated 170908 by Arthur Sugden to combine axonal segmentation in
...
for j = 1:nic
    ...
    % ---------------------------------------------------------------------
    % Inserted for axons, RR and AS
    % Recombine to one axon - centroid is that of largest piece and
    % the sum of all areas
    if axon && ~isempty(Lbig) %AL added the ~isempty(Lbig) 171122
        L = L > 0;
        Lbig = 1;
        ind_largest = find(Larea == max(Larea));
        Lcent = Lcent(ind_largest);
        Larea = sum(struct2array(regionprops(L, 'area')));

        for jj = 1:length(Lbig)
            try segcentroid(jj+k,:) = Lcent(Lbig(jj)).Centroid;
            catch err
               segcentroid(jj+k,:) = [1 1];
            end
        end
    else
    % ---------------------------------------------------------------------
